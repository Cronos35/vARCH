        ...
        public void StartListen()
        {
            ...
            if (_filters != null && _filters.Length > 0)
            {
                for (int i = 0; i < _filters.Length; i++)
                {
                    ...
                    switch (_filters[i].Type)
                    {
                        case WasapiAudioFilterType.LowPass:
                            biQuadSource.Filter = new LowpassFilter(_soundInSource.WaveFormat.SampleRate, _filters[i].Frequency);
                            break;
                        case WasapiAudioFilterType.HighPass:
                            biQuadSource.Filter = new HighpassFilter(_soundInSource.WaveFormat.SampleRate, _filters[i].Frequency);
                            break;
                        case WasapiAudioFilterType.BandPass:
                            biQuadSource.Filter = new BandpassFilter(_soundInSource.WaveFormat.SampleRate, _filters[i].Frequency);
                            break;
                    }
                }
